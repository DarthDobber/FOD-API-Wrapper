...
class fodRequest:
    ...
    def put(self, url, **kwargs):
        ...
        if cfg.use_Proxy:
            if self.c_data and self.c_params:
                r = requests.put(url, headers=self.headers, json=self.json_var, data=self.data, proxies=self.proxyDict)
            elif self.c_params:
                r = requests.put(url, headers=self.headers, json=self.params, proxies=self.proxyDict)
            elif self.c_data:
                r = requests.put(url, headers=self.headers, data=self.data, proxies=self.proxyDict)
            else:
                r = requests.put(url, headers=self.headers, proxies=self.proxyDict)
        # If no proxy server is needed, use these
        else:
            if self.c_data and self.c_params:
                r = requests.put(url, headers=self.headers, json=self.json_var, data=self.data)
            elif self.c_params:
                r = requests.put(url, headers=self.headers, json=self.params)
            elif self.c_data:
                r = requests.put(url, headers=self.headers, data=self.data)
            else:
                r = requests.put(url, headers=self.headers)
        try:
            data = json.loads(r.text)
        except ValueError:
            data = {"Status_code": r.status_code, "Response_Text": r.text}  
        return data
